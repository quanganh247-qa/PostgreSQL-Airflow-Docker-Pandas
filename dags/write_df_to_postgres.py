...
import psycopg2
import traceback
from create_df_and_modify import create_df_from_dtb, creditscore_df, exited_salary_correlation, exited_age_correlation
...
def write_df_to_postgres_main():
    main_df = create_df_from_dtb(cur)
    df_creditscore = create_df_from_dtb(main_df)
    df_exited_age_correlation = exited_age_correlation(main_df)
    df_exited_salary_correlation = exited_salary_correlation(main_df)
